...
-- First, create the missing view for player dashboard metrics
CREATE OR REPLACE VIEW vw_player_dashboard_metrics AS
SELECT 
  p.id as player_id,
  p.user_id,
...
-- Now update the RPC function to use correct player-team relationships
CREATE OR REPLACE FUNCTION public.rpc_get_player_dashboard_v2(target_player_id uuid)
RETURNS json
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Build the comprehensive dashboard data
