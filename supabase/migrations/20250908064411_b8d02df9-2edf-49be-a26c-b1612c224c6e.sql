-- Create membership management system tables and functions (fixed version)
...
DROP POLICY IF EXISTS "All authenticated users can view active membership types" ON public.membership_types;
DROP POLICY IF EXISTS "Super admins can manage membership types" ON public.membership_types;
...
-- Drop existing policies if they exist and recreate them for player_memberships
DROP POLICY IF EXISTS "Players can view their own memberships" ON public.player_memberships;
DROP POLICY IF EXISTS "Parents can view their children's memberships" ON public.player_memberships;
DROP POLICY IF EXISTS "Staff and medical can view all memberships" ON public.player_memberships;
DROP POLICY IF EXISTS "Super admins can manage all memberships" ON public.player_memberships;
...
-- Drop existing policies if they exist and recreate them for membership_usage
DROP POLICY IF EXISTS "Staff can manage membership usage" ON public.membership_usage;
DROP POLICY IF EXISTS "Players can view their own usage" ON public.membership_usage;
...
-- Create view for membership usage summary (drop if exists first)
...
-- Insert some default membership types (only if there are no existing types)
INSERT INTO public.membership_types (name, allocation_type, allocated_classes, start_date_required, end_date_required, created_by) 
SELECT '10 Class Package', 'CLASS_COUNT', 10, true, false, (SELECT id FROM public.profiles LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = '10 Class Package');

INSERT INTO public.membership_types (name, allocation_type, allocated_classes, start_date_required, end_date_required, created_by) 
SELECT '20 Class Package', 'CLASS_COUNT', 20, true, false, (SELECT id FROM public.profiles LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = '20 Class Package');

INSERT INTO public.membership_types (name, allocation_type, allocated_classes, start_date_required, end_date_required, created_by) 
SELECT 'Unlimited Monthly', 'UNLIMITED', NULL, true, false, (SELECT id FROM public.profiles LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = 'Unlimited Monthly');

INSERT INTO public.membership_types (name, allocation_type, allocated_classes, start_date_required, end_date_required, created_by) 
SELECT 'Summer Camp Pass', 'DATE_RANGE', NULL, true, true, (SELECT id FROM public.profiles LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = 'Summer Camp Pass');