...
-- 2) PLAYER MEMBERSHIPS: fix remaining_classes column
...
DO $$
BEGIN
  -- First try to drop the expression (if it's generated)
  BEGIN
    EXECUTE 'ALTER TABLE player_memberships ALTER COLUMN remaining_classes DROP EXPRESSION';
  EXCEPTION WHEN OTHERS THEN
    -- If it fails, the column might not be generated, continue
    NULL;
  END;
  
  -- Ensure it's a regular int column
  BEGIN
    EXECUTE 'ALTER TABLE player_memberships ALTER COLUMN remaining_classes TYPE int';
  EXCEPTION WHEN OTHERS THEN
    -- If column doesn't exist, add it
    EXECUTE 'ALTER TABLE player_memberships ADD COLUMN remaining_classes int';
  END;
END$$;
