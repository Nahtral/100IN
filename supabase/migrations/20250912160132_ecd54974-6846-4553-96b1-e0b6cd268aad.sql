...
-- Fix the 3 Security Definer Views by adding proper RLS
-- First, let's identify and secure the problematic views

-- Secure v_partner_summary view
ALTER VIEW public.v_partner_summary SET (security_invoker = on);

-- Add RLS policies for views that need them
...
-- Create secure policies for partner data
CREATE POLICY IF NOT EXISTS "Partners can view partner data safe"
...
-- Fix Function Search Path warnings by updating functions
