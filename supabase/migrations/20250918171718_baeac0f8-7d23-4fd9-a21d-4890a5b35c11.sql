...
-- Step 3: Function to pick active membership for a player (using correct column names)
...
AS $$
  ...
  WHERE pm.player_id = p_player 
    AND pm.status = 'ACTIVE'
    AND (pm.end_date IS NULL OR pm.end_date >= CURRENT_DATE)
    AND pm.classes_total > pm.classes_used
  ORDER BY COALESCE(pm.end_date, '9999-12-31'::date), (pm.classes_total - pm.classes_used) DESC
  LIMIT 1;
$$;
