-- Create team_members table (source of truth for team membership)
...
-- Create attendance table (separate from player_attendance)
...
CREATE INDEX IF NOT EXISTS idx_attendance_team_event ON public.attendance(team_id, event_id);
CREATE INDEX IF NOT EXISTS idx_attendance_player_event ON public.attendance(player_id, event_id);

-- Create player_memberships table (simple credit model)
...
CREATE INDEX IF NOT EXISTS idx_pm_player_active ON public.player_memberships(player_id) WHERE (is_active);

-- Create membership_ledger table (audit trail)
...
CREATE INDEX IF NOT EXISTS idx_ledger_player_event ON public.membership_ledger(player_id, event_id);
...
-- Create atomic RPC function for saving attendance and deducting credits
...
BEGIN
  ...
  LOOP
    ...
    -- Upsert attendance (unique event+player)
    ...
    -- Deduct one credit ONLY if marked present and not already credited for this event
    credited := FALSE;
    IF v_status = 'present' THEN
      ...
      IF NOT already_counted THEN
        -- Try to deduct from an active membership
        ...
        -- Record ledger entry even if no credits available
...
GRANT EXECUTE ON FUNCTION public.rpc_save_attendance_batch(UUID, UUID, JSONB) TO authenticated;
...
CREATE OR REPLACE VIEW public.v_team_attendance_summary AS
SELECT 
  team_id,
  COUNT(*) FILTER (WHERE status='present') AS present_count,
  COUNT(*) FILTER (WHERE status='late') AS late_count,
