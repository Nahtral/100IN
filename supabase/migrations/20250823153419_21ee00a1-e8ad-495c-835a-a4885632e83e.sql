-- Fix foreign key relationships for chat functionality
-- Add foreign keys that are missing

-- Add foreign key for chat_participants.user_id to profiles.id
ALTER TABLE public.chat_participants 
ADD CONSTRAINT chat_participants_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Add foreign key for chat_participants.chat_id to chats.id
ALTER TABLE public.chat_participants 
ADD CONSTRAINT chat_participants_chat_id_fkey 
FOREIGN KEY (chat_id) REFERENCES public.chats(id) ON DELETE CASCADE;

-- Add foreign key for messages.sender_id to profiles.id
ALTER TABLE public.messages 
ADD CONSTRAINT messages_sender_id_fkey 
FOREIGN KEY (sender_id) REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Add foreign key for messages.chat_id to chats.id
ALTER TABLE public.messages 
ADD CONSTRAINT messages_chat_id_fkey 
FOREIGN KEY (chat_id) REFERENCES public.chats(id) ON DELETE CASCADE;

-- Add foreign key for message_reactions.user_id to profiles.id
ALTER TABLE public.message_reactions 
ADD CONSTRAINT message_reactions_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Add foreign key for message_reactions.message_id to messages.id
ALTER TABLE public.message_reactions 
...
DROP POLICY IF EXISTS "Users can recall their own messages" ON public.messages;
CREATE POLICY "Users can recall their own messages" 
ON public.messages 
FOR UPDATE 
