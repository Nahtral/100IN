-- Phase 1: Fix Player Creation Logic (Skip duplicate constraint, continue with rest)

-- Step 1: Fix current data issues - deactivate unapproved players
...
-- Step 2: Create function to check if user can have player record
...
-- Step 3: Create trigger to prevent player record creation for non-approved users
...
-- Step 4: Create function to automatically create player record when user gets approved
...
-- Step 5: Create function to check email uniqueness (for frontend validation)
