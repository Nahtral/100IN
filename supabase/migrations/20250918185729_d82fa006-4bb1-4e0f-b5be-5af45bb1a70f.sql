...
-- 2) PLAYER MEMBERSHIPS: fix remaining_classes column (remove identity if exists)
ALTER TABLE player_memberships 
ADD COLUMN IF NOT EXISTS override_class_count int,
ADD COLUMN IF NOT EXISTS remaining_classes int,
ADD COLUMN IF NOT EXISTS auto_deactivate_when_used_up boolean DEFAULT true;

-- Remove identity constraint from remaining_classes if it exists
DO $$
BEGIN
  BEGIN
    EXECUTE 'ALTER TABLE player_memberships ALTER COLUMN remaining_classes DROP IDENTITY IF EXISTS';
  EXCEPTION WHEN OTHERS THEN
    -- Ignore if not identity column
    NULL;
  END;
END$$;

-- Ensure remaining_classes has no default (trigger will set it)
ALTER TABLE player_memberships 
ALTER COLUMN remaining_classes DROP DEFAULT;
