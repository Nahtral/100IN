-- Comprehensive Attendance & Membership Fix
-- This migration addresses all schema conflicts and ambiguous references

-- Step 1: Simplify RPC function to ONLY handle attendance (no membership logic)
  ...
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') AND v_new_status = 'present' AND v_old_status != 'present' THEN
    ...
    IF NOT ledger_exists THEN
      ...
      WHERE pm.player_id = NEW.player_id 
        ...
        AND (pm.allocated_classes - pm.classes_used) > 0;

      -- Log the deduction
      INSERT INTO public.membership_ledger(player_id, event_id, delta, reason, created_at)
      VALUES (NEW.player_id, NEW.event_id, -1, 'attendance', now());
    END IF;
  END IF;
...
-- Step 3: Fix membership view to align with auth.users.id schema
...
SELECT 
  ...
  pm.classes_used,
  (pm.allocated_classes - pm.classes_used) as remaining_classes,
  CASE 
    WHEN pm.end_date IS NOT NULL AND pm.end_date < CURRENT_DATE THEN 
      EXTRACT(EPOCH FROM (CURRENT_DATE - pm.end_date))::int / 86400
    WHEN pm.end_date IS NOT NULL THEN 
      EXTRACT(EPOCH FROM (pm.end_date - CURRENT_DATE))::int / 86400
    ELSE NULL
  END as days_left,
  CASE 
    WHEN (pm.allocated_classes - pm.classes_used) <= 0 THEN true
...
INSERT INTO public.membership_types (name, allocation_type, allocated_classes, duration_months, price, is_active, created_by)
VALUES 
  ('Basic Package', 'CLASS_COUNT', 8, 1, 8000, true, auth.uid()),
  ('Premium Package', 'CLASS_COUNT', 16, 2, 15000, true, auth.uid()),
  ('Unlimited Monthly', 'UNLIMITED', NULL, 1, 12000, true, auth.uid())
