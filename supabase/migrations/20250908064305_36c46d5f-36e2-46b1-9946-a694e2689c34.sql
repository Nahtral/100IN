-- Create membership management system tables and functions
...
-- Add RLS policies for membership_types
...
-- Add RLS policies for player_memberships
...
-- Add RLS policies for membership_usage
...
SELECT 
  ...
  pm.player_id,
  p.profiles->>'full_name' as player_name,
...
GROUP BY pm.id, pm.player_id, p.profiles, mt.name, mt.allocation_type, mt.allocated_classes, 
...
INSERT INTO public.membership_types (name, allocation_type, allocated_classes, start_date_required, end_date_required, created_by) VALUES
('10 Class Package', 'CLASS_COUNT', 10, true, false, (SELECT id FROM auth.users LIMIT 1)),
('20 Class Package', 'CLASS_COUNT', 20, true, false, (SELECT id FROM auth.users LIMIT 1)),
('Unlimited Monthly', 'UNLIMITED', NULL, true, false, (SELECT id FROM auth.users LIMIT 1)),
('Summer Camp Pass', 'DATE_RANGE', NULL, true, true, (SELECT id FROM auth.users LIMIT 1))
