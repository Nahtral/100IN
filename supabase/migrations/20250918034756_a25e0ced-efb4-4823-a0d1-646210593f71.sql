-- Comprehensive Attendance & Membership Fix (Schema Corrected)
...
-- Step 3: Fix membership view to align with correct schema
...
-- Step 5: Create sample membership types for testing (need to check schema first)
DO $$ 
BEGIN
  -- Check if membership_types has price column
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'membership_types' AND column_name = 'price'
  ) THEN
