...
-- Create view for partner summary data
...
GROUP BY po.id, po.name, po.partner_type, po.status, po.contact_name, 
         po.contact_email, po.contact_phone, po.website, po.notes, 
...
BEGIN
  ...
  IF partner_id IS NULL THEN
    ...
    INSERT INTO public.partner_organizations (
      name, partner_type, status, contact_name, contact_email, 
      contact_phone, website, notes
    )
    VALUES (
      partner_name, partner_type, partner_status, contact_name, 
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'active_partners', (SELECT COUNT(*) FROM public.partner_organizations WHERE status = 'active'),
