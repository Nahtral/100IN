-- Phase 3: Complete remaining audit fixes (fixed syntax)
...
-- Add missing RLS policies (drop first if they exist, then create)
DROP POLICY IF EXISTS "Super admins can delete teams" ON teams;
CREATE POLICY "Super admins can delete teams" 
ON teams FOR DELETE 
USING (is_super_admin(auth.uid()));

DROP POLICY IF EXISTS "Super admins can delete schedules" ON schedules;
