-- Final Comprehensive Attendance & Membership Fix
-- This migration addresses all schema conflicts and ambiguous references

-- Step 1: Drop and recreate RPC function 
...
-- Step 2: Create single membership deduction trigger
...
-- Step 3: Drop and recreate membership view 
...
-- Step 5: Create sample membership types for testing (remove if already exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = 'Basic Package') THEN
    INSERT INTO public.membership_types (name, allocation_type, allocated_classes, is_active, created_at, updated_at)
    VALUES ('Basic Package', 'CLASS_COUNT', 8, true, now(), now());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = 'Premium Package') THEN
    INSERT INTO public.membership_types (name, allocation_type, allocated_classes, is_active, created_at, updated_at)
    VALUES ('Premium Package', 'CLASS_COUNT', 16, true, now(), now());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.membership_types WHERE name = 'Unlimited Monthly') THEN
    INSERT INTO public.membership_types (name, allocation_type, allocated_classes, is_active, created_at, updated_at)
    VALUES ('Unlimited Monthly', 'UNLIMITED', NULL, true, now(), now());
  END IF;
END $$;

-- Step 6: Update RPC upsert attendance function 
