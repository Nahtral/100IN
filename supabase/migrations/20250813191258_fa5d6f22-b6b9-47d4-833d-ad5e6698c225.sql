...
-- Create medical_clearance permission for more granular control
...
-- Add audit logging trigger for medical data access (for HIPAA compliance)
...
-- Apply audit triggers to medical tables
CREATE TRIGGER audit_daily_health_checkins
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.daily_health_checkins
  FOR EACH ROW EXECUTE FUNCTION public.log_medical_access();

CREATE TRIGGER audit_injury_reports
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.injury_reports
  FOR EACH ROW EXECUTE FUNCTION public.log_medical_access();

CREATE TRIGGER audit_medical_appointments  
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.medical_appointments
  FOR EACH ROW EXECUTE FUNCTION public.log_medical_access();

CREATE TRIGGER audit_health_wellness
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.health_wellness
  FOR EACH ROW EXECUTE FUNCTION public.log_medical_access();