-- Registration visibility hardening - minimal version
-- Focus only on trigger hardening, view/RPC creation

-- 1) Harden onboarding trigger: SECURITY DEFINER + exception handling + idempotent
-- ONLY create profiles, no automatic player creation
...
-- 2) Create pending users view (includes requested role lookup)
