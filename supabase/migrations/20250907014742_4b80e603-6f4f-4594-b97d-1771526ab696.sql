-- Fix the assign_user_role function to match current schema
...
BEGIN
  ...
  -- Insert or update user role with all required columns
...
-- Also create a safer bulk conversion function that handles all edge cases
  ...
  LOOP
    BEGIN
      ...
      SELECT EXISTS(
        ...
        AND role IN ('super_admin', 'staff', 'coach', 'medical', 'partner') 
        AND is_active = true
      ) INTO has_admin_role;
