...
-- 1. Players can view their own data (keep existing)
-- This policy already exists and is correct

-- 2. Parents can view their children's data (keep existing)  
-- This policy already exists and is correct

-- 3. Coaches can only view players from their assigned teams (keep existing)
-- This policy already exists and is correct
...
-- 5. Staff and super admins can manage players (keep existing - already has full access)
-- This policy already exists and is correct
...
USING (
  ...
  EXISTS (
    ...
    WHERE po.contact_user_id = auth.uid()
...
BEGIN
  ...
  IF auth.uid() != NEW.user_id THEN
    ...
    VALUES (
      ...
      'player_data_access',
      jsonb_build_object(
        'accessed_player_id', NEW.id,
        'accessed_player_user_id', NEW.user_id,
        'operation', TG_OP,
        'contains_medical_notes', (NEW.medical_notes IS NOT NULL AND NEW.medical_notes != ''),
        'contains_emergency_contact', (NEW.emergency_contact_name IS NOT NULL AND NEW.emergency_contact_name != ''),
