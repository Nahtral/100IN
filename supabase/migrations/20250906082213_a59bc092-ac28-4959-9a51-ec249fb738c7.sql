-- Clear existing test data and reseed with comprehensive realistic data
-- This ensures we don't have duplicates and provides fresh, realistic data
...
-- Now insert comprehensive realistic data
...
-- Shot tracking data (50 shots per player across multiple sessions)
INSERT INTO shots (
  player_id,
  session_id,
  shot_number,
...
SELECT 
  ...
  (-3 + random() * 6)::numeric,
  CASE 
    WHEN random() < 0.7 THEN 'practice'
    ELSE 'game'
  END,
  determine_shot_region((random() * 800)::numeric, (random() * 600)::numeric),
  NOW() - (random() * INTERVAL '30 days')
FROM players p
CROSS JOIN generate_series(1, 50) shot_num;
