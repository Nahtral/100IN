...
-- 1. Ensure attendance table structure
...
-- 3. Membership ledger for tracking all credit changes
...
-- 4. Trigger function to deduct credits when attendance is marked present
...
-- Attendance policies
CREATE POLICY "attendance_read_access" ON public.attendance
FOR SELECT USING (
...
CREATE POLICY "attendance_write_access" ON public.attendance
FOR ALL USING (
  is_super_admin(auth.uid()) OR 
  has_role(auth.uid(), 'coach'::user_role) OR 
  has_role(auth.uid(), 'staff'::user_role)
...
-- Membership policies
CREATE POLICY "membership_read_own" ON public.player_memberships
FOR SELECT USING (
  player_id = auth.uid() OR 
  is_super_admin(auth.uid()) OR 
  has_role(auth.uid(), 'staff'::user_role)
...
CREATE POLICY "membership_manage_admin" ON public.player_memberships
FOR ALL USING (
  is_super_admin(auth.uid()) OR 
  has_role(auth.uid(), 'staff'::user_role)
);
