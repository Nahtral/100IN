...
SELECT 
  ...
  CASE 
    WHEN EXISTS(SELECT 1 FROM public.user_roles ur WHERE ur.user_id = au.id AND ur.role = 'super_admin' AND ur.is_active = true) 
    THEN 'approved'
    ELSE 'pending'
  END
FROM auth.users au
...
-- Ensure all users have at least a player role
...
SELECT 
  ...
  now(),
  (SELECT id FROM public.profiles WHERE approval_status = 'approved' AND EXISTS(SELECT 1 FROM public.user_roles WHERE user_id = profiles.id AND role = 'super_admin') LIMIT 1),
  CASE WHEN p.approval_status = 'approved' THEN now() ELSE NULL END
FROM public.profiles p
