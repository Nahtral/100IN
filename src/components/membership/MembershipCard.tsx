...
export const MembershipCard: React.FC<MembershipCardProps> = ({
  ...
  const { adjustUsage, loading: adjustLoading } = useAdjustUsage();
  ...
  return (
    <>
      ...
      {adjustModalOpen && summary && (
        <AdjustUsageModal
          isOpen={adjustModalOpen}
          onClose={() => setAdjustModalOpen(false)}
          membershipSummary={summary}
          onAdjust={async (delta: number, reason: string) => {
            const success = await adjustUsage(summary.membership_id, delta, reason);
            if (success && onAdjustUsage) {
              onAdjustUsage();
            }
            return success;
          }}
